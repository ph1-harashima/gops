-- Prototype PostgreSQL only. Phase 9-B: Import Folder Integration
-- (Production-Oriented PO Workflow §13 Phase 2). No schema change other
-- than the Audit Event allow-list - the Import Folder hand-off itself
-- writes only to local Filesystem (never a DB row of its own; success/
-- failure is recorded entirely via official_po_integration_request.status,
-- already present since V9/V18) and to audit_event via the two new types
-- below.
